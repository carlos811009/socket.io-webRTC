...
export class AppComponent implements OnInit {
  ...
  initPeer() {
    ...
    try {
      ...
      this.localPeer.addEventListener('addstream', (event: any) => {
        console.log('in addListen', event.stream)
        this.remoteStream = event.stream
        const videoTrack = event.stream.getVideoTracks()
        this.remoteIsOpenVideo = !!videoTrack.length
        console.log('remoteStream', videoTrack)
        setTimeout(() => {
          this.showRemote()
        }, 1000)
      })
  ...
  // async onCreateOffer(desc: any) {
  //   try {
  //     await this.remotePeer.setLocalDescription(desc); // 呼叫端设置本地 offer 描述
  //   } catch (e) {
  //     console.log('Offer-setLocalDescription: ', e);
  //   }
  //   try {
  //     await this.localPeer.setRemoteDescription(desc); // 接收端设置远程 offer 描述
  //   } catch (e) {
  //     console.log('Offer-setRemoteDescription: ', e);
  //   }
  //   try {
  //     let answer = await this.localPeer.createAnswer(); // 接收端创建 answer
  ...
  // async onCreateAnswer(desc: any) {
  //   try {
  //     await this.localPeer.setLocalDescription(desc); // 接收端设置本地 answer 描述
  //   } catch (e) {
  //     console.log('answer-setLocalDescription: ', e);
  //   }
  //   try {
  //     await this.remotePeer.setRemoteDescription(desc); // 呼叫端端设置远程 answer 描述
  //   } catch (e) {
  ...
  async onOffer(data: any) {
    try {
      // console.log('onOffer', this.socketId, data.sdp)
      // 接收端设置远程 offer 描述
